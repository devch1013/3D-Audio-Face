...
class EmoTalk(nn.Module):
    def __init__(self, args):
        ...
        self.feature_dim = args["ttf_param"]["feature_dim"]
        self.bs_dim = args["ttf_param"]["bs_dim"]
        self.device = args["general"]["device"]
        self.batch_size = args["general"]["batch_size"]
        ...
        self.max_seq_len = args["ttf_param"]["max_seq_len"]
        ...
        self.biased_mask1 = init_biased_mask(
            n_head=4,
            max_seq_len=args["ttf_param"]["max_seq_len"],
            period=args["ttf_param"]["period"],
        )
        ...
        decoder_layer = nn.TransformerDecoderLayer(
            d_model=args["ttf_param"]["feature_dim"],
            nhead=4,
            dim_feedforward=args["ttf_param"]["feature_dim"],
            batch_first=True,
        )
    ...
    def forward(self, data):
        ...
        inputs12 = self.processor(
            torch.squeeze(data["input12"]),
            sampling_rate=16000,
            return_tensors="pt",
            padding="longest",
        ).input_values.to(self.device)
        hidden_states_cont1 = self.audio_encoder_cont(
            inputs12, frame_num=frame_num11
        ).last_hidden_state
        hidden_states_cont12 = self.audio_encoder_cont(
            inputs12, frame_num=frame_num12
        ).last_hidden_state
        inputs21 = self.feature_extractor(
            torch.squeeze(data["input21"]), sampling_rate=16000, padding=True, return_tensors="pt"
        ).input_values.to(self.device)
        inputs12 = self.feature_extractor(
            torch.squeeze(data["input12"]), sampling_rate=16000, padding=True, return_tensors="pt"
        ).input_values.to(self.device)
        ...
        if data["target11"].shape[0] == 1:
            ...
            obj_embedding_person = (
                self.obj_vector_person(onehot_person).unsqueeze(0).permute(1, 0, 2)
            )
        ...
        hidden_states11 = torch.cat(
            [
                hidden_states_cont1,
                hidden_states_emo11_256,
                obj_embedding_level11,
                obj_embedding_person11,
            ],
            dim=2,
        )
        ...
        hidden_states12 = torch.cat(
            [
                ...
                hidden_states_emo12_256,
                obj_embedding_level12,
                obj_embedding_person12,
            ],
            dim=2,
        )
        if data["target11"].shape[0] == 1:
            tgt_mask11 = (
                self.biased_mask1[:, : hidden_states11.shape[1], : hidden_states11.shape[1]]
                .clone()
                .detach()
                .to(device=self.device)
            )
            tgt_mask22 = (
                self.biased_mask1[:, : hidden_states12.shape[1], : hidden_states12.shape[1]]
                .clone()
                .detach()
                .to(device=self.device)
            )

        memory_mask11 = enc_dec_mask(
            self.device, hidden_states11.shape[1], hidden_states11.shape[1]
        )
        memory_mask12 = enc_dec_mask(
            self.device, hidden_states12.shape[1], hidden_states12.shape[1]
        )
        bs_out11 = self.transformer_decoder(
            hidden_states11, hidden_states_emo11_832, tgt_mask=tgt_mask11, memory_mask=memory_mask11
        )
        bs_out12 = self.transformer_decoder(
            hidden_states12, hidden_states_emo12_832, tgt_mask=tgt_mask22, memory_mask=memory_mask12
        )
    ...
    def predict(self, audio, level, person):
        ...
        inputs12 = self.processor(
            torch.squeeze(audio), sampling_rate=16000, return_tensors="pt", padding="longest"
        ).input_values.to(self.device)
        hidden_states_cont1 = self.audio_encoder_cont(
            inputs12, frame_num=frame_num11
        ).last_hidden_state
        inputs12 = self.feature_extractor(
            torch.squeeze(audio), sampling_rate=16000, padding=True, return_tensors="pt"
        ).input_values.to(self.device)
