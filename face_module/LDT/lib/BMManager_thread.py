...
class MyMesh():
    ...
    def LoadMesh(self):
        ...
        vertex = 6706
        face = 13120
        self.Vertices = np.zeros((vertex, 6), dtype = np.float32)
        self.Normals = np.zeros((vertex, 3), dtype = np.float32)
        self.Rotations = np.zeros((vertex, 3, 3), dtype = np.float32)
        self.Faces = np.zeros((face, 7), dtype = np.int32)
        self.UV = np.zeros((vertex, 2), dtype = np.float32)
        ...
        for line in f:
            ...
            if (words[0] == "vt" and idx_uv<vertex):
                self.UV[idx_uv] = [words[1], words[2]]
                idx_uv+=1
                
            if (words[0] == "f" and idx_f<face):
                ...
                # print(self.Faces[idx_f])
                # print(idx_f, self.Faces[idx_f])
...
class BMMng():
    ...
    def LoadBS(self):
        ...
        currMesh = MyMesh(self.path + "generic_neutral_mesh.obj")
        currMesh.LoadMesh()
        ...
        for i in tqdm(range(len(self.blend_name))):
            ...
            try:
                ...
                currMesh = MyMesh(self.path + self.blend_name[i] + ".obj")
                currMesh.LoadMesh()
            # print(currMesh)
            self.BlendShapes.append(currMesh)
    ...
    def RescaleAvatar(self):
        ...
        self.AVATAR_LANDMARKS = previous_landmarks
    ...
    def DeformationTransfer(self):
        ...
        def make_mesh_thread(meshnum):
            ...
            self.Avatar[meshnum+1] = currMesh
        ...
        for meshnum in tqdm(range(len(self.blend_name))):
            thread = threading.Thread(target=make_mesh_thread, args=[meshnum])
            thread.start()
            threads.append(thread)
        for idx, thread in enumerate(threads):
            thread.join()
            print(f"{idx}/{len(threads)}")
