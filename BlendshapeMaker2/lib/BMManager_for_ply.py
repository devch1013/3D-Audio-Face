...
class MyMesh():
    ...
    def LoadMeshPly(self):
        ...
        for line in f:
            ...
            if(words[0] == "comment"):
                if(words[1] == "TextureFile"):
                    self.TexNameSource += [words[2]]
                continue

            if(words[0] == "element"):
                if(words[1] == "vertex"):
                    num_v = int(words[2])
                    self.VerticesSource = np.zeros((int(words[2]), 4), dtype=np.float32)
                elif(words[1] == "face"):
                    num_f = int(words[2])
                    self.FacesSource = np.zeros((int(words[2]), 7), dtype=np.int32)
                    self.TexturesSource = np.zeros((int(words[2]), 6), dtype=np.float32)
                continue

            if(words[0] == "property"):
                if(words[1] == "list" and words[2] == "uchar" and words[3] == "float" and words[4] == "texcoord"):
                    self.FlagsSource[0] = True
                continue
        ...
        for line in f:
            ...
            self.VerticesSource[idx_v] = [float(words[0]), float(words[1]), float(words[2]), idx_v]
        ...
        for line in f:
            words = line.split()
            if(self.FlagsSource[0]):
                self.FacesSource[idx_f] = np.array([int(words[1]), 0, int(words[2]), 0,  int(words[3]), 0, int(words[11])])
                self.TexturesSource[idx_f, :] = words[5:11]
            else:
                self.FacesSource[idx_f, 0:6] = np.array([int(words[1]), 0, int(words[2]), 0,  int(words[3]), 0])
            # test
            # print "Face %d" %(idx_f), self.Faces[idx_f]
...
class dtf():
    def __init__(self, corrList, SOURCE_LANDMARKS, AVATAR_LANDMARKS, SourceExpression, SourceNeutral, AvatarNeutral,
        ...
        self.SE_Vertices = SourceExpression.VerticesSource
        self.SN_Vertices = SourceNeutral.VerticesSource
        self.AN_Vertices = AvatarNeutral.Vertices
        self.Vertices = AvatarNeutral.Vertices
        self.SE_Faces = SourceExpression.FacesSource
        self.SN_Faces = SourceNeutral.FacesSource
...
class BMMng():
    ...
    def LoadBS(self):
        ...
        currMesh = MyMesh(self.path + "neutral.ply")
        currMesh.LoadMeshPly()
        self.BlendShapes.append(currMesh)

        for i in range(401):
            
            currMesh = MyMesh(self.path + "sample_mesh" + str(i) + ".ply")
            currMesh.LoadMeshPly()
            self.BlendShapes.append(currMesh)
    ...
    def RescaleAvatar(self):
        ...
        def select_landmarks(reference_landmarks, previous_landmarks):
            ...
            Source = Mesh_VFN(self.BlendShapes[0].VerticesSource, self.BlendShapes[0].FacesSource)
        ...
        ''''''''''''''''''''''''''''''''''''''''''''''''
        # bind nosetop
        xc = Vertices[self.AVATAR_LANDMARKS[13], 0] - self.BlendShapes[0].VerticesSource[self.SOURCE_LANDMARKS[13]][0]
        yc = Vertices[self.AVATAR_LANDMARKS[13], 1] - self.BlendShapes[0].VerticesSource[self.SOURCE_LANDMARKS[13]][1]
        zc = Vertices[self.AVATAR_LANDMARKS[13], 2] - self.BlendShapes[0].VerticesSource[self.SOURCE_LANDMARKS[13]][2]
        k = 0
        for v in Vertices:
            v[0] = v[0]-xc
            v[1] = v[1]-yc
    ...
    def TriangleCorrespondence(self, level):
        ...
        SourceFaces = self.BlendShapes[0].FacesSource
        SourceVertices = self.BlendShapes[0].VerticesSource
    ...
    def DeformationTransfer(self):
        ...
        SN_Vertices = self.BlendShapes[0].VerticesSource
