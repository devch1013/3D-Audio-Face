...
class MyMesh():
    ...
    def LoadMesh(self):
        ...
        self.Vertices = np.zeros((6191, 6), dtype=np.float32)
        # self.Normals = np.zeros((int(words[2]), 3), dtype=np.float32)
        # self.Rotations = np.zeros((int(words[2]), 3, 3), dtype=np.float32)
        self.Faces = np.zeros((12346, 7), dtype=np.int32)
        # self.Normals = np.zeros((int(words[2]), 3), dtype=np.float32)
        # self.Rotations = np.zeros((int(words[2]), 3, 3), dtype=np.float32)
        # self.Faces = np.zeros((9976, 7), dtype=np.int32)
        # self.UV = np.zeros((9976, 2), dtype=np.float32)
        # self.V4 = np.zeros((9976, 3), dtype = np.float32)
        ...
        for line in f:
            ...
            if (words[0] == "v"):
                self.Vertices[idx_v] = [float(words[1])*500, float(words[2])*500, float(words[3])*500, idx_v, 0, 0]
                idx_v += 1
            ...
            if (words[0] == "f"):
                self.Faces[idx_f] = np.hstack(
                    (np.array([words[1], 0, words[2], 0, words[3], 0]).reshape(6), 0))
...
class BMMng():
    ...
    def LoadBS(self):
        ...
        currMesh = MyMesh(self.path + "neutral.obj")
        currMesh.LoadMesh()
        self.BlendShapes.append(currMesh)

        for i in range(BLENDSHAPES):
            currMesh = MyMesh(self.path + "mesh" + str(i) + ".obj")
            currMesh.LoadMesh()
            self.BlendShapes.append(currMesh)
    ...
    def RescaleAvatar(self):
        ...
        SourceVerticies = self.BlendShapes[0].Vertices
        ...
        sxs = SourceVerticies[0, 0]
        sxl = xs
        sys = SourceVerticies[0, 1]
        syl = ys
        szs = SourceVerticies[0, 2]
        szl = zs
        ...
        print("SourceSize: x[%f, %f], y[%f, %f], z[%f, %f]" % (sxs, sxl, sys, syl, szs, szl))
        ...
        # Rescale
        sourceavg = (sxl-sxs+syl-sys+szl-szs)/3
    ...
    def DeformationTransfer(self):
        ...
        for meshnum in range(BLENDSHAPES):
