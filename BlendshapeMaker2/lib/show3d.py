...
class Show_Vertices(tk.Frame):
    def DrawMesh(self, meshnum):
        ...
        if(meshnum==0):
            ...
            for i in Face_idxs:
                poly = []
                gaze = (Cam_Pos[0:3] - self.Source.Vertices[self.Source.FacesSourceSource[i,0],0:3]) / LA.norm(Cam_Pos[0:3] - self.Source.Vertices[self.Source.FacesSourceSource[i,0],0:3])
                inner = np.dot(self.Source.Normals[i], gaze)
                if(inner>=0):
                    for k in range(3):
                        poly += [self.Vertices_spix[self.Source.FacesSourceSource[i,2*k],0], self.Vertices_spix[self.Source.FacesSourceSource[i,2*k],1]]
